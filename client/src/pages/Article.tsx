...
export default function Article() {
  ...
  const fallbackSource = [...featuredStories, ...mainFeedStories].find((s) => s.id === id);
  const fallbackArticle: ArticleType | null = fallbackSource
    ? {
        id: fallbackSource.id,
        slug: fallbackSource.id,
        title: fallbackSource.title,
        summary: fallbackSource.description ?? "",
        content: fallbackSource.description ?? "Icerik yakinda eklenecek.",
        category: fallbackSource.category,
        author: fallbackSource.author,
        coverImage: fallbackSource.image,
        isFeatured: false,
  ...
  if (!resolvedArticle) return <div>Makale bulunamadi.</div>;
      ...
      <main>
         ...
         <div className="relative w-full h-[50vh] md:h-[60vh] bg-muted">
            <div className="absolute inset-0">
                 <img 
                 src={resolvedArticle.coverImage}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
               ...
               <div className="hidden lg:block lg:col-span-2 space-y-6 sticky top-24 self-start">
                  <div className="flex flex-col gap-1">
                     ...
                     <span className="font-serif font-bold text-lg text-primary">{resolvedArticle.author}</span>
                  </div>
                  <div className="flex flex-col gap-1">
                     <span className="text-xs uppercase tracking-widest text-muted-foreground font-bold">Tarih</span>
                     <span className="font-mono text-sm">{new Date(resolvedArticle.publishedAt).toLocaleDateString("tr-TR")}</span>
                  </div>
